<script lang="ts">
  ...
  import { Plus, Check, Truck, Eye } from 'lucide-svelte';
      ...
      <table class="w-full">
        ...
        <tbody class="divide-y divide-slate-100">
          {#if filteredOrders.length === 0}
            <tr><td colspan="6" class="text-center py-12 text-slate-400 text-sm">暂无销售订单</td></tr>
          {:else}
      ...
      <div class="p-6">
        ...
        <div class="space-y-4">
          ...
          <div class="flex items-center justify-between pt-3 border-t border-slate-200">
            <p class="text-sm text-slate-500">合计：<span class="font-bold text-slate-800">{formatCurrency(newItems.reduce((s, i) => s + i.quantity * i.price, 0))}</span></p>
